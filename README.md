...
- Level 3 is Zombie Village Run: the supplied zombie-villager walk, idle, attack and death animations are rasterized into local transparent sprite sheets. The villager chases from behind in the first half, then appears on the right and attacks left at timed intervals. Raised platforms are safe escape routes during its strike window, and a deterministic drawn fallback keeps the encounter stable if an asset is unavailable.
